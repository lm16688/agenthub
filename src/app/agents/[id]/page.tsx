...
export default function AgentDetailPage() {
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/agents/${params.id}/like`, {
        method,
      });
      ...
      if (res.ok) {
        ...
        setAgent(agent ? {
          ...agent,
          likesCount: agent.likesCount + (liked ? -1 : 1)
        } : null);
      }
    } catch (error) {
  ...
  const handleBookmark = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/agents/${params.id}/bookmark`, {
        method,
      });
  ...
  if (error || !agent) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center">
        <div className="text-white text-xl">{error || "Agent not found"}</div>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-4 mb-4">
            ...
            <Badge variant={agent.isFree ? "default" : "secondary"}>
              {agent.isFree ? "免费" : `¥${agent.price}`}
            </Badge>
          </div>
          <div className="text-gray-400 mb-4">{agent.description}</div>
          <div className="flex items-center gap-6 text-sm text-gray-400">
            <span>作者: {agent.author}</span>
            <span>分类: {agent.category}</span>
            <span>版本: {agent.versionsCount}</span>
            <span>❤️ {agent.likesCount}</span>
          </div>
        </div>
        ...
        <Card className="bg-white/5 border-white/10 p-6">
          <h2 className="text-2xl font-semibold mb-4">版本历史</h2>
          <p className="text-gray-400">版本信息加载中...</p>
        </Card>
      </div>
    </div>
  );
